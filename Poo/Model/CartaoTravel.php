...
class CartaoTravel {
   ...
   public function cadastrarCartao($t){
    ...
    public function encerrarCartao(){
       if($this->getSaldo()>0){
           echo "<p>O cartão travel ainde tem dinheiro, não pode encerrar.</p>";
       }else{
    ...
    public function usarCartao($valor){
       if($this->getSituacao()==TRUE){
           if($this->getSaldo() >= $valor){
               $this->setSaldo($this->getSaldo()-$valor);
               echo "<p>Pagamento de R$ $valor autorizado ao cartão travel de".$this->getUsuario()."</p>";
           }else{
   ...
   public function __construct() {
       $this->setSaldo(0);
       $this->setSituacao(FALSE);
       echo "<p>Conta do cartão travel criada cem sucesso!</p>";
   }
